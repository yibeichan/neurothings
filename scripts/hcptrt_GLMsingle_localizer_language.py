...
def get_binary_df(df: pd.DataFrame, n_tr: int, sorted_unique_trial_types: List[str], stimdur: float, tr: float) -> pd.DataFrame:
    """
    Create a binary DataFrame representing stimulus occurrences, splitting long trials into multiple shorter ones.
    ...
    Args:
        ...
        duration (float): Target duration for each trial segment in seconds.
    ...
    # Calculate the minimum buffer in TRs (8 seconds worth of TRs at the end)
...
def load_event_and_json_files(event_files: List[str], json_files: List[str], brain_files: List[str], tr: float, stimdur: float) -> List[np.ndarray]:
    ...
    for i, (event_file, json_file) in enumerate(zip(event_files, json_files)):
        try:
            ...
            df = df.dropna(subset=["stim_file", "stim_file_question1"], how="all")
...
def main(sub_id: str, task: str, tr: float, n_jobs: int, event_files: List[str], json_files: List[str], brain_files: List[str], output_dir: str):
    ...
    stimdur = 3
    design_matrices = load_event_and_json_files(event_files, json_files, brain_files, tr, stimdur)
    ...
    results_dir = os.path.join(output_dir, f"results_v2/{sub_id}/{task}")
